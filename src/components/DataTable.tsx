...
import { findMissingAsins } from '../utils/dataProcessing';
...
export const DataTable: React.FC<DataTableProps> = ({ 
  ...
  const missingAsins = comparisonData ? findMissingAsins(data, comparisonData) : [];
      ...
      {comparisonData && (
        <div className="bg-white rounded-lg shadow-md overflow-hidden">
          ...
          <div className="p-4 space-y-4">
            {/* Missing ASINs Section */}
            <div className="bg-gray-50 rounded-lg p-4">
              <h4 className="text-sm font-semibold text-gray-700 mb-2">
                ASINs Missing from PBI Data {missingAsins.length > 0 && `(${missingAsins.length})`}
              </h4>
              {missingAsins.length > 0 ? (
                <div className="flex flex-wrap gap-2">
                  {missingAsins.map(asin => (
                    <span key={asin} className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-yellow-100 text-yellow-800">
                      {asin}
                    </span>
                  ))}
                </div>
              ) : (
                <p className="text-sm text-gray-600">No missing ASINs found</p>
              )}
            </div>
